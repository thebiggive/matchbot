 ...
 *@throws NotFoundException
    ...
    private function filterForSearch(
        ...
        foreach ($jsonMatchInListConditions as $field => $value) {
            if (! preg_match('/^[a-z_]+$/i', $field)) {
                throw new \InvalidArgumentException("Invalid JSON match field '$field'");
            }
    ...
    /**
     * @param list<string>|null $locationsOfInterest
     * @param QueryBuilder $qb Builder with its select etc. already set up.
     * @param bool $sortByLocation
    ...
    private function sortForSearch(
        ...
        bool $sortByLocation,
        array|null $locationsOfInterest
    ): void {
        ...
        if ($safeSortField === 'relevance') {
            ...
            $qb->setParameter('orderedIds', $idsOrderedByRelavence);
                ...
                Assertion::notNull($locationsOfInterest) && Assertion::minCount($locationsOfInterest, 1); // @mago-expect analysis:possibly-null-argument

                // @phpstan-ignore function.alreadyNarrowedType
                Assertion::true(\array_is_list($locationsOfInterest)); // @mago-expect analysis:redundant-type-comparison - required for security, otherwise the _$i below would be an SQL injection vulnerability

                // Each following location along the list will just count for half of the previous ones so it works as a binary number with the various levels of region specificty acting
                // as different plave values. We assume there won't be more than 10 levels of nested locations.
                $locationRank = 2 ^ 10; // the first location in the list of those of interest to the searcher is most important, so should dominate the search results by counting for 1024.
                Assertion::maxCount($locationsOfInterest, 10);

                // location matching code originally AI-generated by Junie/Geminie - and then I used AI chat to explain it. It's
                // going through all the locations of interest, and doing a +1 to a add to a score if a level of the location
                // of each campaign matches it - but if somehow the campaign has duplicates of the same location then it still
                // only gets a a +1, not a +2 or more. Definsive coding against potential of confused data from SF with
            ...
            foreach ($locationsOfInterest as $i => $regionCode) {
                $locationMatchCountSelect .= "CASE WHEN (SUM(CASE WHEN campaignLocation.regionCode = :regionCode_{$i} THEN {$locationRank} ELSE 0 END) > 0) THEN {$locationRank} ELSE 0 END + ";
                $qb->setParameter("regionCode_{$i}", $regionCode);
                $locationRank = (int) ($locationRank / 2);
            }
