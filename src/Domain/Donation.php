...
class Donation extends SalesforceWriteProxy
{
    ...
    /**
     * If changing this make sure to update copy in FE to match
     * (currently says 'Match funds are reserved for 25 minutes while online') or make that be determined from this.
     *
     * This is the maximum time that donors expect to can keep match funds reserved as long they stay online. In the
     * minute after this expires we auto remove the matching from donations, but they can still continue the donation
     * if they POST a message to us to say they are happy to donate unmatched.
     *
     * And if they go offline the donation will expire much quicker (currently after five minutes).
     */
    public static function expiryInterval(): \DateInterval
    {
        return match (Environment::current()) {
            Environment::Production, Environment::Regression, Environment::Test => new \DateInterval('PT25M'),
            Environment::Staging, Environment::Local => new \DateInterval('PT10M'),
        };
    }
    ...
    private(set) ?DateTimeImmutable $fundsReservedUntil = null;
    ...
    public function extendReservationFrom(DateTimeImmutable $at): void
    {
        ...
        $this->fundsReservedUntil = min($at->modify('+5 minutes'), $this->maxReservationTime());
    }

    /**
     * Returns the latest possible time that funds may be reserved for this donation. Currently based on the
     * time the donation was created, in future may also take into account the campaign closing time.
     * @return DateTimeImmutable
     */
    private function maxReservationTime(): \DateTimeImmutable
    {
        return $this->getCreatedDateImmutable()->add(Donation::expiryInterval());
    }
}
